...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!error && residents && residents.length > 0) {
      console.log(`✅ Resident Search: Found ${residents.length} live records.`);
      return res.status(200).json({ success: true, residents, source: 'cloud_supabase' });
    }
  } catch (cloudError) {
    console.warn('⚠️ Search Resilience: Supabase inactive - attempting local resolution...');
  }
