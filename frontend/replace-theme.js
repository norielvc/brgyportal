...
if (startIndex !== -1 && endIndex !== -1) {
    ...
    section = section.replace(/from-\[#1e1b4b\] via-\[#312e81\] to-\[#1e1b4b\]/g, 'from-[#0a1f12] via-[#113821] to-[#0a1f12]');
    ...
    section = section.replace(/hover:bg-indigo-900\/50/g, 'hover:bg-[#113821]/60');

    // Update all hover borders to gold glow
    section = section.replace(/hover:border-[a-z]+-400\/40/g, 'hover:border-[#d4af37]/50 hover:shadow-[0_0_20px_rgba(212,175,55,0.2)]');
    ...
    section = section.replace(/bg-yellow-500 text-indigo-900/g, 'bg-gradient-to-r from-[#d4af37] to-[#aa8c2c] text-[#0a1f12]');
    section = section.replace(/bg-[a-z]+-500 text-white/g, 'bg-gradient-to-r from-[#d4af37] to-[#aa8c2c] text-[#0a1f12]');

    // Icon containers
    section = section.replace(/bg-gradient-to-br from-[a-z]+-600 to-[a-z]+-400/g, 'bg-gradient-to-br from-[#d4af37] to-[#ebd78c]');
    section = section.replace(/shadow-\[0_0_15px_rgba\([0-9,]+,0\.3\)\]/g, 'shadow-[0_0_15px_rgba(212,175,55,0.4)]');
    ...
    section = section.replace(/group-hover:text-yellow-400/g, 'group-hover:text-[#ebd78c]');
    section = section.replace(/group-hover:text-emerald-400/g, 'group-hover:text-[#ebd78c]');
    section = section.replace(/group-hover:text-blue-400/g, 'group-hover:text-[#ebd78c]');
    section = section.replace(/group-hover:text-red-400/g, 'group-hover:text-[#ebd78c]');
    section = section.replace(/group-hover:text-pink-400/g, 'group-hover:text-[#ebd78c]');
    section = section.replace(/group-hover:text-purple-400/g, 'group-hover:text-[#ebd78c]');
    ...
    section = section.replace(/\$\{selectedAchievement\.colorClass\}/g, 'bg-gradient-to-r from-[#d4af37] to-[#aa8c2c] text-[#0a1f12]');
    ...
    section = section.replace(/className={`text- text-sm md:text-base font-bold tracking-widest uppercase mb-2`}/g, 'className={`text-[#d4af37] text-sm md:text-base font-bold tracking-widest uppercase mb-2`}');
