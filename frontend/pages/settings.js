...
export default function Settings() {
  ...
  const fetchSignatures = async () => {
    ...
    try {
      const token = getAuthToken();
      console.log(
        "Fetching signatures with token:",
      ...
      if (data.success) {
        ...
        if (data.signatures && data.signatures.length > 0) {
          ...
          console.log(
            ...
            data.signatures[0].signature_data ? "Present" : "Missing",
          );
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/residents/search?name=&page=1&limit=100000`,
      );
      const data = await response.json();
      ...
      if (data.success && data.residents.length > 0) {
        ...
        XLSX.writeFile(
          wb,
          `Residents_Database_Export_${new Date().toISOString().split("T")[0]}.xlsx`,
        );
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      {/* Messages */}
      {successMessage && (
        <div className="flex items-center p-3.5 sm:p-4 bg-green-50 border border-green-200 rounded-xl text-xs sm:text-sm">
          <CheckCircle className="w-4 h-4 sm:w-5 sm:h-5 text-green-600 mr-2.5 shrink-0" />
          <p className="text-green-700 font-medium">{successMessage}</p>
        </div>
      )}

      {errorMessage && (
        <div className="flex items-center p-3.5 sm:p-4 bg-red-50 border border-red-200 rounded-xl text-xs sm:text-sm">
          <AlertCircle className="w-4 h-4 sm:w-5 sm:h-5 text-red-600 mr-2.5 shrink-0" />
      ...
      <div className="bg-white rounded-xl sm:rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="border-b border-gray-100">
          <div className="flex overflow-x-auto no-scrollbar gap-2 sm:gap-6 px-3 sm:px-6">
            {tabs.map((tab) => {
              const Icon = tab.icon;
              return (
                <button
                  ...
                  className={`py-3 sm:py-4 px-2 sm:px-1 border-b-2 font-bold text-xs sm:text-sm transition-all whitespace-nowrap shrink-0 ${
                    activeTab === tab.id
          ...
          {activeTab === "security" && (
            <div className="space-y-6 max-w-2xl">
              ...
              <div>
                ...
                />
                <p className="text-sm text-gray-600 mt-1">
                  Force users to change password after this period
                </p>
              </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-sm text-blue-800">
                    ...
                    <ul className="space-y-1 text-blue-700">
                      <li>
                        • Your signatures are securely stored and encrypted
                      </li>
                      <li>
                        • Default signature will be used automatically in forms
                      </li>
                      <li>
                        • You can create multiple signatures for different
                        purposes
                      </li>
                      <li>
                        • Signatures are legally binding for barangay documents
                      </li>
                    </ul>
                  </div>
                </div>
            ...
            <div className="space-y-4 max-w-2xl">
              {[
                ...
                },
                {
                  key: "loginAlerts",
                  label: "Login Alerts",
                  desc: "Alert on new login attempts",
                },
                {
                  key: "securityAlerts",
                  label: "Security Alerts",
                  desc: "Alert on security events",
                },
                {
                  key: "weeklyReports",
                  label: "Weekly Reports",
                  desc: "Receive weekly system reports",
                },
              ].map((item) => (
                <div
                  key={item.key}
              ...
              <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      Download the entire resident directory as an Excel file
                ...
                >
                  ...
                  {isExporting
                    ...
                    : "DOWNLOAD RESIDENTS DIRECTORY (XLSX)"}
                </button>
              </div>
              ...
              <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500">
                      Quickly upload multiple resident records using a formatted
                      CSV file.
                    </p>
                  </div>
                </div>
                    ...
                    <div className="overflow-x-auto rounded-xl border border-gray-200 shadow-sm bg-white">
                      <table className="min-w-full divide-y divide-gray-100">
                        <thead className="bg-gray-50">
                          <tr>
                            <th className="px-4 py-2.5 text-left text-[9px] font-black text-gray-500 uppercase tracking-tight">
                              First Name
                            </th>
                            <th className="px-4 py-2.5 text-left text-[9px] font-black text-gray-500 uppercase tracking-tight">
                              Last Name
