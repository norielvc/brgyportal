...
export default function AchievementsPage() {
  ...
  const saveAllChanges = async () => {
    try {
      ...
      if (data.success) {
        ...
        setNotification({
          type: "error",
          message: data.message || "Failed to save achievements",
        });
      }
    } catch (error) {
      ...
      setNotification({
        type: "error",
        message: "Failed to save achievements. Please try again.",
      });
    } finally {
      setSaving(false);
    }
  ...
  const addAchievement = () => {
    ...
    setNotification({
      type: "success",
      message: "Achievement added. Remember to Save All.",
    });
  };
  ...
  const saveEdit = () => {
    if (
      !editingAchievement.title.trim() ||
      !editingAchievement.category.trim()
    ) {
      setNotification({
        type: "error",
    ...
    setNotification({
      type: "success",
      message: 'Achievement updated locally. Click "Save All" to publish.',
    });
  };
  ...
  const deleteAchievement = (id) => {
    ...
    setNotification({
      type: "success",
      message: 'Achievement removed. Click "Save All" to publish.',
    });
  };
  ...
  const moveAchievement = (index, direction) => {
    ...
    [newAchievements[index], newAchievements[newIndex]] = [
      newAchievements[newIndex],
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            Barangay Achievements & Awards
          </h1>
          <p className="text-gray-600 mt-1">
            Configure awards displayed on the homepage
          </p>
        </div>
      ...
      {hasChanges && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <span className="text-amber-800 font-medium">
            Unsaved changes. Click "Save All" to publish.
          </span>
        </div>
      )}
              ...
              <div className="flex flex-col md:flex-row">
                ...
                <div className="flex-1 p-4">
                  <div className="flex justify-between items-start">
                    <div>
                      <span
                        className={`text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded ${ach.color_class} text-white`}
                      >
                        {ach.category}
                      </span>
                    ...
                    <div className="flex flex-col gap-1">
                      <button
                        ...
                        disabled={index === 0}
                        className={`p-1 rounded ${index === 0 ? "text-gray-200" : "text-gray-400 hover:bg-gray-100"}`}
                      >
                      ...
                      <button
                        ...
                        className={`p-1 rounded ${index === achievements.length - 1 ? "text-gray-200" : "text-gray-400 hover:bg-gray-100"}`}
                      >
            ...
            {achievements.length > 0 ? (
              <div className="relative group rounded-xl overflow-hidden border border-white/10">
                ...
                <div className="absolute bottom-4 left-4 right-4">
                  <p className="text-yellow-500 text-[8px] font-bold uppercase tracking-widest">
            ...
            <div className="mt-8 bg-black/30 rounded-xl p-4 border border-white/5">
              ...
              <ul className="text-xs text-white/60 space-y-2">
                <li className="flex gap-2">
                  • <span>Use high-quality photos (16:9 recommended)</span>
                </li>
                <li className="flex gap-2">
                  • <span>Keep descriptions punchy and impactful</span>
                </li>
                <li className="flex gap-2">
                  • <span>Update years to reflect current records</span>
                </li>
              </ul>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div
                      className="relative group h-48 rounded-xl overflow-hidden border-2 border-dashed border-gray-200 hover:border-blue-500 transition-colors cursor-pointer"
            ...
            <div className="p-6 border-t border-gray-100 flex gap-4 bg-gray-50/50">
              <button
                onClick={saveEdit}
                className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-black uppercase tracking-widest text-xs flex items-center justify-center gap-2 shadow-lg shadow-blue-600/20"
              >
                <Check className="w-4 h-4" /> Save Changes
              </button>
              <button
                onClick={() => setEditingAchievement(null)}
                className="flex-1 bg-white border border-gray-200 text-gray-500 hover:bg-gray-50 py-3 rounded-xl font-black uppercase tracking-widest text-xs"
              >
                Cancel
              </button>
            </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 border-t border-gray-100 flex gap-4 bg-gray-50/50">
              <button
                ...
                className="flex-1 bg-green-600 hover:bg-green-700 text-white py-3 rounded-xl font-black uppercase tracking-widest text-xs flex items-center justify-center gap-2 shadow-lg shadow-green-600/20"
              >
