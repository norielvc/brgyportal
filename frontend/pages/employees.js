...
export default function Employees() {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-3 sm:gap-4 items-stretch md:items-center justify-between pb-4 sm:pb-6 border-b border-gray-100">
        <div className="relative w-full md:w-1/3 group">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 text-gray-400 group-focus-within:text-blue-500 transition-colors" />
          <input
            type="text"
            placeholder="SEARCH PERSONNEL..."
            className="w-full pl-11 pr-4 py-2.5 sm:py-3.5 bg-gray-50 border-2 border-transparent rounded-xl sm:rounded-2xl focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all font-black text-gray-900 uppercase text-[11px] sm:text-xs tracking-tight placeholder:font-black placeholder:text-gray-300 shadow-inner"
            value={searchTerm}
            onChange={handleSearch}
          />
        </div>
        <div className="flex items-center gap-2.5 sm:gap-4 w-full md:w-auto">
          <div className="bg-gray-50 px-3 sm:px-4 py-2 rounded-xl border border-gray-100 flex items-center gap-2.5 shrink-0">
            ...
            <div>
              <p className="text-[8px] sm:text-[9px] font-black text-gray-400 uppercase tracking-widest leading-none mb-0.5">
                Staff
              </p>
              <p className="text-xs sm:text-[15px] font-black text-gray-800 tracking-tighter leading-none">
                {employees.length || 0}
              </p>
            </div>
          </div>
          <button
            ...
            className="flex-1 sm:flex-none px-4 sm:px-8 py-2.5 sm:py-3.5 bg-blue-600 text-white rounded-xl sm:rounded-2xl text-[10px] sm:text-[11px] font-black uppercase tracking-wider sm:tracking-[0.2em] shadow-lg sm:shadow-xl shadow-blue-200 hover:bg-blue-700 active:scale-95 transition-all flex items-center justify-center gap-2"
          >
      ...
      {/* Error State */}
      {error && (
        <div className="bg-rose-50 border-2 border-rose-100 rounded-2xl sm:rounded-3xl p-4 sm:p-6 flex items-start gap-3 sm:gap-4 animate-in fade-in slide-in-from-top-4">
          <div className="bg-rose-100 p-2 sm:p-2.5 rounded-xl shrink-0">
      ...
      {/* Content Area */}
      <div className="bg-white border border-gray-100 rounded-2xl sm:rounded-[2.5rem] shadow-sm overflow-hidden min-h-[300px] sm:min-h-[500px] flex flex-col">
        {isLoading ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center py-14 sm:py-20 bg-gray-50/50 p-4">
            <div className="bg-white p-4 sm:p-6 rounded-2xl sm:rounded-3xl border border-gray-100 shadow-xl mb-4 sm:mb-6 opacity-50">
              <UsersIcon className="w-12 h-12 sm:w-16 sm:h-16 text-gray-300" />
            </div>
            <h3 className="text-base sm:text-xl font-black text-gray-900 uppercase tracking-tight text-center">
              No Personnel Found
            </h3>
            <p className="text-[10px] sm:text-[11px] font-bold text-gray-400 uppercase tracking-widest mt-1 mb-6 text-center">
              Try adjusting your identification filters
            </p>
            <button
              onClick={() => setShowAddModal(true)}
              className="px-6 sm:px-10 py-3 sm:py-4 bg-gray-900 text-white rounded-xl sm:rounded-2xl font-black text-[10px] sm:text-[11px] uppercase tracking-widest hover:bg-black transition-all shadow-xl shadow-gray-200"
            >
          ...
          <>
            {/* Mobile Cards View */}
            <div className="block md:hidden divide-y divide-gray-100 p-2 sm:p-3 space-y-2.5">
              {filteredEmployees.map((emp) => (
                <div
                  key={emp._id}
                  className="bg-white p-3.5 rounded-xl border border-gray-100 shadow-2xs space-y-3 hover:border-blue-200 transition-all"
                >
                  <div className="flex items-start justify-between gap-2.5">
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div className="relative shrink-0">
                        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-50 to-blue-50 border border-blue-100 flex items-center justify-center text-blue-600 font-black text-sm shadow-2xs">
                          {emp.firstName?.[0]}
                          {emp.lastName?.[0]}
                        </div>
                        <div
                          className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-white ${emp.status === "active" ? "bg-emerald-500" : "bg-gray-300"}`}
                        ></div>
                      </div>
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          {emp.employeeCode && (
                            <span className="px-1.5 py-0.2 bg-blue-50 text-blue-600 border border-blue-100 rounded text-[8px] font-black tracking-tighter">
                              {emp.employeeCode}
                            </span>
                          )}
                        </div>
                    ...
                    <span
                      className={`inline-flex items-center px-2 py-0.5 rounded-md text-[8px] font-black tracking-widest uppercase border shrink-0 ${emp.role === "admin" ? "bg-indigo-50 text-indigo-700 border-indigo-100" : "bg-blue-50 text-blue-700 border-blue-100"}`}
                    >
                      {emp.role}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-gray-50 text-[10px]">
                      ...
                      <div className="flex items-center gap-1">
                        {emp.status === "active" ? (
                          <>
                            ...
                            <span className="font-black text-emerald-700 text-[9px] uppercase">
                              Active
                            </span>
                          </>
                        ) : (
                          <>
                            ...
                            <span className="font-black text-rose-700 text-[9px] uppercase">
                              Suspended
                            </span>
                          </>
                        )}
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="p-1.5 bg-gray-50 hover:bg-blue-50 text-gray-500 hover:text-blue-600 rounded-lg transition-colors"
                        title="View"
                      >
                      ...
                      <button
                        ...
                        className="p-1.5 bg-gray-50 hover:bg-indigo-50 text-gray-500 hover:text-indigo-600 rounded-lg transition-colors"
                        title="Edit"
                      >
                      ...
                      <button
                        ...
                        className="p-1.5 bg-gray-50 hover:bg-amber-50 text-gray-500 hover:text-amber-600 rounded-lg transition-colors"
                        title="Reset"
                      >
                      ...
                      <button
                        ...
                        className="p-1.5 bg-gray-50 hover:bg-rose-50 text-gray-500 hover:text-rose-600 rounded-lg transition-colors"
                        title="Delete"
                      >
      ...
      {!isLoading && employees.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 sm:gap-6">
          {[
            ...
            <div
              key={i}
              className="bg-white p-3 sm:p-5 rounded-xl sm:rounded-3xl border border-gray-100 shadow-sm flex items-center gap-2.5 sm:gap-4"
            >
              <div className={`p-2 sm:p-3 rounded-lg sm:rounded-2xl bg-gray-50 ${stat.color} shrink-0`}>
                <stat.icon className="w-4 h-4 sm:w-5 sm:h-5" />
              </div>
              <div className="min-w-0">
                <p className="text-[8px] sm:text-[9px] font-black text-gray-400 uppercase tracking-wider truncate">
                  {stat.label}
                </p>
                <p
                  className={`text-base sm:text-xl font-black ${stat.color} tracking-tight`}
                >
                  {stat.value}
                </p>
              </div>
