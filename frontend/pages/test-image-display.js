...
export default function TestImageDisplay() {
  ...
  return (
    <Layout
      ...
      subtitle="Debug signature image display issues"
    >
                      ...
                      <div className="w-32 h-16 border border-gray-300 rounded bg-white overflow-hidden">
                        {sig.signature_data ? (
                          <img
                            ...
                            onLoad={() => {
                              console.log(
                                `✅ Database signature ${index + 1} loaded successfully`,
                              );
                            }}
                            onError={(e) => {
                              console.error(
                                `❌ Database signature ${index + 1} failed to load`,
                              );
                              console.error("Error details:", {
                                name: sig.name,
                                dataLength: sig.signature_data?.length,
                                dataStart: sig.signature_data?.substring(
                                  0,
                                  100,
                                ),
                                isValidDataURL:
                                ...
                                hasBase64:
                                  sig.signature_data?.includes("base64,"),
                              });
                        ...
                        <div
                          ...
                          style={{
                            display: sig.signature_data ? "none" : "flex",
                          }}
                        >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-gray-600 space-y-1">
                        <p>
                          ...
                          {sig.signature_data ? "Yes" : "No"}
                        </p>
                        {sig.signature_data && (
                          <>
                            <p>
                              <strong>Data Length:</strong>{" "}
                              {sig.signature_data.length}
                            </p>
                            <p>
                              <strong>Valid Data URL:</strong>{" "}
                              {sig.signature_data.startsWith("data:image/")
                                ? "Yes"
                                : "No"}
                            </p>
                            <p>
                              <strong>Has Base64:</strong>{" "}
                              {sig.signature_data.includes("base64,")
                                ? "Yes"
                                : "No"}
                            </p>
