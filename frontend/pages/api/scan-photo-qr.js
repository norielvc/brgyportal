...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.body && (req.body.base64 || req.body.image || req.body.photo)) {
      const b64 = req.body.base64 || req.body.image || req.body.photo;
      const cleanB64 = b64.replace(/^data:image\/\w+;base64,/, "");
      inputBuffer = Buffer.from(cleanB64, "base64");
    } else if (Buffer.isBuffer(req.body)) {
      inputBuffer = req.body;
    }

    if (!inputBuffer) {
      return res.status(400).json({ success: false, qrText: null, debug: "no_image_data" });
    }

    debug.push(`size:${Math.round(inputBuffer.length / 1024)}KB`);

    // Auto-orient EXIF and scale to 800px max (fast, crisp, low memory)
    ...
    console.error("Scan photo QR server error:", err);
