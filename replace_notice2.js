...
files.forEach(file => {
    ...
    // A more robust replacement: look for <div className="bg-gradient-to-br from-emerald-50 to-teal-50... 
    const startIndex = content.indexOf('<div className="bg-gradient-to-br from-emerald-50 to-teal-50');
    if (startIndex === -1) return;

    // Find where the next sibling block starts. It usually starts with `{submitStatus` or `<form` or `<div className="space-y-4` or `<div className="space-y-6`
    let nextBlockMatches = [
        content.indexOf('<div className="space-y-4', startIndex),
        content.indexOf('<div className="space-y-6', startIndex),
        content.indexOf('{submitStatus === \'error\'', startIndex),
        content.indexOf('<form className="', startIndex),
        content.indexOf('<div className="space-y-', startIndex)
    ].filter(i => i !== -1);

    const endIndex = Math.min(...nextBlockMatches);
    if (endIndex === Infinity) {
        console.log("Could not find end index in " + file);
        return;
    }

    content = content.substring(0, startIndex) + newNoticeBlock + "\n            " + content.substring(endIndex);
